...
"""
Settings > General 에서 '턴 구분자', 'Q&A 구분자' 에 줄바꿈 기호를 넣어서 적용하려면 어떻게 해야 하나요? '\n'를 추가하면 줄바꿈으로 인식하지 않고 문자열로 인식하네요.                                                                             
C:\JnJ\Developments\Projects\@chrome-extension\jnj-obsidian-clipper\src\managers\general-settings.ts 에 있는  turnSepInput, qaSepInput 변수를 참고해주세요
"""
...
수정후에는 '\n'이 문자열로도 안나오고, 줄바꿈도 안되네요. 그리고 '# CDP와 Chrome Extension============' 와 같이, '============' 앞에 있던 '\n'가 없어지기만 해요.
"""

  ┌──────┬────────────────────────────────────┬─────────────────────────────────┐
  │      │         이전 (내 첫 수정)          │            이번 수정            │
  ├──────┼────────────────────────────────────┼─────────────────────────────────┤
  │ 저장 │ \n → 실제 줄바꿈으로 변환 후 저장  │ 입력 그대로 저장 (\n 리터럴)    │
  ├──────┼────────────────────────────────────┼─────────────────────────────────┤
  │ 로드 │ 실제 줄바꿈 → \n으로 변환해서 표시 │ 저장된 그대로 표시              │
  ├──────┼────────────────────────────────────┼─────────────────────────────────┤
  │ 사용 │ 저장된 실제 줄바꿈 그대로 사용     │ 이 시점에 \n → 실제 줄바꿈 변환 │
  └──────┴────────────────────────────────────┴─────────────────────────────────┘

  핵심 차이: 변환 위치를 "저장/로드 시"에서 "노트 생성 시"로 옮겼습니다.

  ---
  사용 방법:
  1. Settings > General 에서 '턴 구분자' 또는 'Q&A 구분자' 필드에 \n=== 형태로 입력
  2. \n은 리터럴 텍스트로 저장되고, 노트 생성 시 실제 줄바꿈으로 변환됩니다

  기존에 이미 실제 newline으로 저장된 값 (내 첫 번째 수정으로 인해)은 필드에서 === 앞에 공백처럼 보이거나 비어 보일 수      
  있습니다. 해당 필드를 \n=== 형태로 다시 입력하고 저장하시면 됩니다.